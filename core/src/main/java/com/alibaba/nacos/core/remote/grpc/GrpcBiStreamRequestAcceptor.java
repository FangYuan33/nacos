...
public class GrpcBiStreamRequestAcceptor extends BiRequestStreamGrpc.BiRequestStreamImplBase {
    ...
    public StreamObserver<Payload> requestBiStream(StreamObserver<Payload> responseObserver) {
        // [clientConnection] 步骤20：服务端接收客户端双向流连接请求，创建流观察器
        StreamObserver<Payload> streamObserver = new StreamObserver<>() {
            ...
            public void onNext(Payload payload) {
                // [clientConnection] 步骤21：服务端接收客户端发送的数据包，进行解析处理
    ...
    private void streamObserverOnNext(Payload payload, String connectionId, Integer localPort, int remotePort,
        ...
        Object parseObj;
        try {
            // [clientConnection] 步骤22：解析客户端发送的数据包，提取请求对象
        ...
        if (parseObj instanceof ConnectionSetupRequest) {
            // [clientConnection] 步骤23：处理连接建立请求，提取客户端信息
            ...
            // [clientConnection] 步骤24：创建连接元信息，包含客户端IP、端口、版本等
            ...
            // [clientConnection] 步骤25：创建服务端连接对象，绑定响应流和通道
            ...
            if (setUpRequest.getAbilityTable() != null) {
                // map to table
                // [clientConnection] 步骤26：设置客户端能力表，用于功能协商
            ...
            if (rejectSdkOnStarting || !connectionManager.register(connectionId, connection)) {
                ...
                // [clientConnection] 步骤28：连接注册成功，发送能力协商响应给客户端
                try {
                    ...
                    if (setUpRequest.getAbilityTable() != null) {
                        ...
                        // [clientConnection] 步骤29：发送SetupAck响应，告知客户端连接建立成功
